...
impl<N: Network, C: ConsensusStorage<N>> Validator<N, C> {
    ...
    fn read_committee(dev: Option<u16>) -> (NarwhalKeyPair, Committee) {
        ...
        let primary_keypair = read_authority_keypair_from_file(primary_key_file).unwrap_or_else(|_| {
            panic!("Failed to load the node's primary keypair {base_path}.primary-{primary_id}-key.json")
        });
