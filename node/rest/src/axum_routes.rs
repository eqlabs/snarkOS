...
pub(crate) async fn latest_height<N: Network, C: ConsensusStorage<N>, R: Routing<N>>(
...
pub(crate) async fn latest_hash<N: Network, C: ConsensusStorage<N>, R: Routing<N>>(
    State(rest): State<Rest<N, C, R>>,
) -> Json<N::BlockHash> {
...
pub(crate) async fn latest_block<N: Network, C: ConsensusStorage<N>, R: Routing<N>>(
    State(rest): State<Rest<N, C, R>>,
) -> Json<Block<N>> {
...
pub(crate) async fn latest_state_root<N: Network, C: ConsensusStorage<N>, R: Routing<N>>(
    State(rest): State<Rest<N, C, R>>,
) -> Json<N::StateRoot> {
...
pub(crate) async fn get_block<N: Network, C: ConsensusStorage<N>, R: Routing<N>>(
    State(rest): State<Rest<N, C, R>>,
...
pub(crate) async fn get_blocks<N: Network, C: ConsensusStorage<N>, R: Routing<N>>(
    State(rest): State<Rest<N, C, R>>,
...
pub(crate) async fn get_height<N: Network, C: ConsensusStorage<N>, R: Routing<N>>(
    State(rest): State<Rest<N, C, R>>,
...
pub(crate) async fn get_block_transactions<N: Network, C: ConsensusStorage<N>, R: Routing<N>>(
    State(rest): State<Rest<N, C, R>>,
...
pub(crate) async fn get_transaction<N: Network, C: ConsensusStorage<N>, R: Routing<N>>(
    State(rest): State<Rest<N, C, R>>,
...
pub(crate) async fn get_memory_pool_transactions<N: Network, C: ConsensusStorage<N>, R: Routing<N>>(
    State(rest): State<Rest<N, C, R>>,
...
pub(crate) async fn get_program<N: Network, C: ConsensusStorage<N>, R: Routing<N>>(
    State(rest): State<Rest<N, C, R>>,
...
pub(crate) async fn get_state_path_for_commitment<N: Network, C: ConsensusStorage<N>, R: Routing<N>>(
    State(rest): State<Rest<N, C, R>>,
...
pub(crate) async fn get_beacons<N: Network, C: ConsensusStorage<N>, R: Routing<N>>(
    State(rest): State<Rest<N, C, R>>,
) -> Result<Json<Vec<Address<N>>>, RestError> {
    match rest.consensus {
...
pub(crate) async fn get_peers_count<N: Network, C: ConsensusStorage<N>, R: Routing<N>>(
    State(rest): State<Rest<N, C, R>>,
) -> Json<usize> {
...
pub(crate) async fn get_peers_all<N: Network, C: ConsensusStorage<N>, R: Routing<N>>(
    State(rest): State<Rest<N, C, R>>,
) -> Json<Vec<SocketAddr>> {
...
pub(crate) async fn get_peers_all_metrics<N: Network, C: ConsensusStorage<N>, R: Routing<N>>(
    State(rest): State<Rest<N, C, R>>,
...
pub(crate) async fn get_node_address<N: Network, C: ConsensusStorage<N>, R: Routing<N>>(
    State(rest): State<Rest<N, C, R>>,
) -> Json<Address<N>> {
...
pub(crate) async fn find_block_hash<N: Network, C: ConsensusStorage<N>, R: Routing<N>>(
    State(rest): State<Rest<N, C, R>>,
    Path(tx_id): Path<N::TransactionID>,
...
pub(crate) async fn find_transaction_id_from_program_id<N: Network, C: ConsensusStorage<N>, R: Routing<N>>(
    State(rest): State<Rest<N, C, R>>,
...
pub(crate) async fn find_transaction_id_from_transition_id<N: Network, C: ConsensusStorage<N>, R: Routing<N>>(
    State(rest): State<Rest<N, C, R>>,
...
pub(crate) async fn find_transition_id<N: Network, C: ConsensusStorage<N>, R: Routing<N>>(
    State(rest): State<Rest<N, C, R>>,
...
pub(crate) async fn transaction_broadcast<N: Network, C: ConsensusStorage<N>, R: Routing<N>>(
    State(rest): State<Rest<N, C, R>>,
